# Changelog
Detailed changelog of this project. You'll find a [short summary](./WHATSNEW.md) of the latest changes here. – _[Legacy builds](#legacy) are listed at the bottom._

---

_In April 2026 we will ship the new **`a7`** line of the app supporting 16 kB pages and Android 15+ transparent bars. It is already available via Github releases, but not yet on the app stores_
...
### Added
- RAM caching for background images to speed up dial creation. Background png saving was moved to a delayed worker to reduce initial render time, and compression was optimised. Dial creation is now significantly faster (2× faster). For the first time, the Android 7+ preview line is faster than the old line
- The Android 7+ preview line now becomes the new **`main`** line ✨. The previous main line becomes the legacy line and now uses the **`a5`** suffix. A separate GitHub repository has been created for the distribution of the legacy apk
...
- App vars weren't copied back to fns vars on first startup after install
- Button page 3 disabled colour when protection mode is enabled
- Wallclocks autocenters dial correctly after 5 min

## [2.71.1-public] - 2026-03-31

### Changed
- Display of patch number on the About page; as a result, the comparison logic was changed to 3+1 parts (hence it was necessary to upgrade to version 2.71, which triggers an update notification for wall clocks)
- Texts updated
...
- Some experiments with an additional solc journal that have been discontinued as they offer no added value
- Texts updated

### Fixed
- More edge option also enlarges the timeshift border
- Weather fix for DST switches and these hours

## [2.70-public] - 2026-03-23 (Time control & Meteor showers)

### Added
- Time control mode added to move the dial forward and backward in time: red border when time shifting, processing animation when calculating. _With version 2.70 for the first time, the Android app includes all core functions of the original Raspberry Pi clock_ 🎉
...
- The app now also ships with the auto-translated language: Polish – _As this feature is experimental and dependent on other services, auto-translated languages may be temporarily or permanently unavailable_
...
- Agnihotra marks on dial were not shown anymore
...
- Weather location mismatch handling refined: within 5 km the weather remains "valid"; only beyond that point (and up to 100 km) it is marked grey (meaning: location drift or if older than 6h)
...
- The "not yet translated" message includes an auto translate link #2.69.1
...
- For auto dark theme & twilight markings: sunrise condition changed from -0.8333° to -0.2667° 
