...
### Added
- From now on, the preview line takes over as the new **`main`** line (without a suffix), while the previous main line becomes the legacy line and now uses the **`a5`** suffix. In the app stores, the previous main line will continue to be distributed as **`a5`** for a limited time. A separate GitHub repository has also been created for distribution of the **`a5`** legacy apk. This supports the toolchain split planned for April 2026
